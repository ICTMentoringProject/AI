...
def get_google_image(max_count, query , group = "") :
  ...
  key = group + " " + query 
  elem.send_keys(key) # 검색창에 검색할 내용(name)넣기
  ...
  for image in images:
    try:
        ...
        urllib.request.urlretrieve(imgUrl, directory + key + "_" + str(count) + ".jpg")
        print("Image saved:" + key + "_" + str(count) + ".jpg")
        count += 1
        if count > max_count:
            break
    except:
...
def get_naver_image(max_count, query , group = "") :
  driver = webdriver.Chrome() # 크롬드라이버 설치한 경로 작성 필요 
  ...
  for image in images:
    try:
        ...
        imgUrl = image.get_attribute('src')
        urllib.request.urlretrieve(imgUrl, directory + key + "_" + str(count) + ".jpg")
        print("Image saved:" + querkeyy + "_" + str(count) + ".jpg")
        count += 1
        if count > max_count:
            break
    except:
